      ...
      <section className="py-24">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div className="bg-card p-6 rounded-2xl border border-border/10">
                  ...
                  <h3 className="font-serif font-bold text-xl mb-2">{t("contact.visitUs")}</h3>
                  <p className="text-muted-foreground leading-relaxed">
                    Refah Gift Market,<br />Industrial Area,<br />Shiebat Al Salam,<br />Abu Dhabi, UAE
                  </p>
                </div>
                <div className="bg-card p-6 rounded-2xl border border-border/10">
                  <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center mb-4 text-primary">
                  ...
                  <h3 className="font-serif font-bold text-xl mb-2">{t("contact.callUs")}</h3>
                  <p className="text-muted-foreground leading-relaxed mb-4">Available during store hours.</p>
                  <a href="tel:+971567772003" className="text-primary font-medium hover:underline">+971 56 777 2003</a>
                </div>
              </div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-card p-8 md:p-10 rounded-2xl border border-border/10">
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  <FormField control={form.control} name="name" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder={t("contact.namePlaceholder")} {...field} className="bg-background" data-testid="input-name" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="email" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder={t("contact.emailPlaceholder")} {...field} className="bg-background" data-testid="input-email" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="message" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder={t("contact.messagePlaceholder")} className="min-h-[150px] bg-background" {...field} data-testid="textarea-message" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <Button type="submit" className="w-full" disabled={createMessage.isPending} data-testid="button-submit">
                    {createMessage.isPending ? t("contact.sending") : (
                      <>{t("contact.submit")} <Send size={16} className="ms-2" /></>
                    )}
                  </Button>
                </form>
              </Form>
